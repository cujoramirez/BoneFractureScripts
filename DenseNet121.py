...
# ---------------------------
NUM_CLASSES = 7
# Even though your dataset is resized to 256x256, we will further resize to 224x224 for DenseNet121.
...
class TransferLearningModel(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        ...
        # Custom classifier head with reduced dropout
    ...
    def unfreeze_layer_group(self, layer_name):
        # For DenseNet121, we unfreeze specific dense blocks:
        if hasattr(self.base_model, 'features'):
            if layer_name == 'layer4':
                # Unfreeze final dense block and norm (indexes 10 and 11)
                for param in self.base_model.features[10].parameters():
                    param.requires_grad = True
                for param in self.base_model.features[11].parameters():
                    param.requires_grad = True
            elif layer_name == 'layer3':
                # Unfreeze third dense block and its transition (indexes 8 and 9)
                for param in self.base_model.features[8].parameters():
                    param.requires_grad = True
                for param in self.base_model.features[9].parameters():
                    param.requires_grad = True
            else:
...
# ---------------------------
# Trainer for Progressive Transfer Learning with FP16 using AdamW, Early Stopping, SWA, etc.
# ---------------------------
class TransferLearningTrainer:
    ...
    def train_phases(self, num_epochs, patience=3):
        best_val_f1 = 0
        ...
        for epoch in range(num_epochs):
            ...
            if val_metrics['f1_score'] > best_val_f1:
                ...
                epochs_without_improvement = 0
                torch.save(self.model.state_dict(), os.path.join(MODEL_SAVE_PATH, 'DenseNet121_best.pth'))
            else:
...
def main():
    ...
    if CROSS_VALIDATE:
        ...
        for train_index, val_index in skf.split(df_train, df_train['label']):
            ...
            # For cross-validation, use the same validation transformation for the validation set
    ...
    if os.path.exists(os.path.join(MODEL_SAVE_PATH, 'DenseNet121_best.pth')):
        best_model_path = os.path.join(MODEL_SAVE_PATH, 'DenseNet121_best.pth')
        model.load_state_dict(torch.load(best_model_path, map_location=DEVICE))
        
        export_model(model, MODEL_SAVE_PATH, 'DenseNet121')
