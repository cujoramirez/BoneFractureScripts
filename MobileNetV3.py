...
# ---------------------------
NUM_CLASSES = 7
MODEL_INPUT_SIZE = 224  # MobileNetV3 expects 224x224 images
BATCH_SIZE = 32
NUM_EPOCHS = 50
...
# ---------------------------
class TransferLearningMobileNetV3Model(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        super(TransferLearningMobileNetV3Model, self).__init__()
    ...
    def unfreeze_last_block(self):
        # Unfreeze the last block of features (assumed to be the last element in base_model.features)
        for param in self.base_model.features[-1].parameters():
            param.requires_grad = True
    
    def unfreeze_last_two_blocks(self):
    # Unfreeze the last two blocks in the features ModuleList
...
# ---------------------------
# Trainer for Progressive Transfer Learning (MobileNetV3)
# ---------------------------
class TransferLearningMobileNetV3Trainer:
    ...
    def setup_optimization(self, phase):
        if phase == 'initial':
            head_params = list(self.model.base_model.classifier[3].parameters())
    ...
    def train_phases(self, num_epochs, patience=EARLY_STOP_PATIENCE):
        ...
        for epoch in range(num_epochs):
            ...
            if val_results['f1_score'] > best_val_f1:
                ...
                epochs_without_improvement = 0
                torch.save(self.model.state_dict(), os.path.join(MODEL_SAVE_PATH, 'MobileNetV3_best.pth'))
            else:
    ...
    def progressive_training(self):
        ...
        print("\nPhase 2: Fine-tuning with last 2 block...")
        # Use the MobileNetV3 method to unfreeze the last 2 block
        self.model.unfreeze_last_two_blocks()  
        self.setup_optimization('fine_tuning')
        self.train_phases(5, patience=EARLY_STOP_PATIENCE)
        
        print("\nPhase 3: Fine-tuning with all layers...")
    ...
    def update_history(self, train_metrics, val_metrics):
        ...
        self.history['train_roc_auc'].append(train_metrics.get('roc_auc', 0))
        ...
        self.history['val_roc_auc'].append(val_metrics.get('roc_auc', 0))
    ...
    def print_epoch_summary(self, epoch, train_metrics, val_metrics):
        # (Printing is handled in train_phases)
        pass
...
def main():
    ...
    if CROSS_VALIDATE:
        ...
        for train_index, val_index in skf.split(df_train, df_train['label']):
            ...
            model = TransferLearningMobileNetV3Model(NUM_CLASSES, pretrained=True)
            model = model.to(DEVICE)
            trainer = TransferLearningMobileNetV3Trainer(model, train_loader, val_loader, None, DEVICE, class_weights=class_weights)
    ...
    if os.path.exists(os.path.join(MODEL_SAVE_PATH, 'MobileNetV3_best.pth')):
        best_model_path = os.path.join(MODEL_SAVE_PATH, 'MobileNetV3_best.pth')
        model.load_state_dict(torch.load(best_model_path, map_location=DEVICE, weights_only=False))
        export_model(model, MODEL_SAVE_PATH, 'MobileNetV3')
    ...
    # Plotting histories and metrics
